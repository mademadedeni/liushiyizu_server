...
let selectArticle = function (params) {
    let _sql = `SELECT t_article.id,title,content,t_article.create_date,t_article.update_date,t_article.permission,
        ...
        FROM t_article INNER JOIN t_user ON t_article.user_id = t_user.id ORDER BY t_article.update_date DESC LIMIT ${params.pageIndex},${params.pageSize}`;
    return query(_sql);
}
...
let deleteArticle = function (id) {
    let _sql = `DELETE FROM t_article WHERE id = ?`;
    return query(_sql,id);
}
